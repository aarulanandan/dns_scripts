...
#creates non-normalized, non-resized .csv files for calculated quantities from extracted quantities
    #variable directories (var_dirs) must be list of full file paths to location of extracted data files
    #expressions (exprs) must be a list of strings of mathematical expressions to be evaluated
    #quantity names (expr_names) must be list of strings representing the names of each quantity in the order of expressions given that will be written out to individual directories
    #variable names (var_names) must be list of characters/names for individual variables (eg. T or Temperature) in order in which directories are given in var_dirs that are used in the expressions provided
    #output directory (out_path) must be subdirectory in which individual directories for each term will be created and populated
def quantity(exprs,expr_names,var_names,var_dirs,out_path):
    #example: quantity(['T*P','0.5*D'],['TP','Dhalf'],['T','P','D'],['/localdata/scratch/adhav/Data/2_H2O2Ar_L16/Temperature/','/localdata/scratch/adhav/Data/2_H2O2Ar_L16/Pressure/','/localdata/scratch/adhav/Data/2_H2O2Ar_L16/Density/'],'/localdata/scratch/adhav/Data/2_H2O2Ar_L16/')
    nterms = len(exprs)
    nvars = len(var_names)

    dinfo = [f for f in os.listdir(var_dirs[0]) if os.path.isfile(os.path.join(var_dirs[0],f))]
    nfiles =  len(dinfo)
    ...
    for n in range(0,nterms):
        ...
        for fi in range(0,nfiles):
            ...
            for m in range(0,nvars):
                ...
                q = np.asarray(pd.read_csv(os.path.join(var_dirs[m],dinfo[fi]),header=None))
                globals()[var_names[m]] = q

            quant = eval(exprs[n])

            if not os.path.isdir(os.path.join(out_path,expr_names[n])):
                os.mkdir(os.path.join(out_path,expr_names[n]))

            filename = expr_names[n]+'/'+(''.join(filter(str.isdigit,str(dinfo[fi]))))+'.csv' 

            np.savetxt(os.path.join(out_path,filename),quant,delimiter=',')

#creates normalized .csv files for source terms
    #shock.csv must exist (find_shock function must be run already)
    #terms (terms) must be list of strings containing mathematical expressions, in which variables are of the form Xavg or Yfluc, etc.
...
def source(Xgrid,Ygrid,cell_width,terms,var_names,avg_files,fluc_files,out_path):
    ...
    for m in range(0,nvars):
        ...
        q = np.asarray(pd.read_csv(avg_files[m],header=None))
        q_1 = q[Ysz,...]
        qavg = q_1[...,Xsz]/vars_norm[m]

        avg_name = var_names[m]+'avg'
        globals()[avg_name] = qavg
        
    for n in range(0,nterms):

        term_ind = terms[n].split(r'[*/| ()+-=]+')
        nterms_ind = len(term_ind)

        os.mkdir(os.path.join(out_path+(''.join(term_ind))))

        for m in range(0,nterms_ind):
            if not any('fluc','avg') in term_ind[m]:
                term_ind[m] = ''

        term_ind = list(filter(None,term_ind))
        nterms_ind = len(term_ind)
        ...
        for m in range(0,nterms_ind):
            ...
            dinfo = [f for f in os.listdir(fluc_files[indx_m]) if os.path.isfile(os.path.join(fluc_files[indx_m],f))]
            dinfon = dinfon.append(dinfo)
        ...
        for fi in range(0,nfiles):
            for m in range(0,nterms_ind):
                if 'fluc' in term_ind[m]:
                    q = np.asarray(pd.read_csv(os.path.join(fluc_files[(indx[m])],dinfon[m][fi],header=None)))
                    q_1 = q[Ysz,...]
                    qfluc = q_1[...,Xsz]
                    
                    fluc_name = term_ind[m][0]+'fluc'
                    globals()[fluc_name] = qfluc
...
#creates plots of normalized spatial distribution of source terms at each time step
    ...
    #output directory (out_path) must be full path to directory in which source term plots will be saved, under a subdirectory named 'plots'
def source_plot(Xgrid,Ygrid,cell_width,term_dirs,term_names,out_path):
    #example: source_plot(4096,4096,1.45,['/localdata/scratch/adhav/Source_Comp_2_H2O2Ar_L16/TflucQfluc/','/localdata/scratch/adhav/Source_Comp_2_H2O2Ar_L16/TflucQavg/'],['Tfluc*Qfluc','Tfluc*Qavg'],'/localdata/scratch/adhav/Source_Comp_2_H2O2Ar_L16/')

    Xsz = np.arange(Xgrid)
    Ysz = np.arange(Ygrid)

    (vars_norm,L_norm,t_norm) = normalize(Xgrid,Ygrid,'',cell_width,'')
    ...
    for n in range(0,nterms):
        ...
        os.mkdir(os.path.join(out_path,'plots',term_names[n]))

        for fi in range(0,nfiles):

            q_raw = np.asarray(pd.read_csv(os.path.join(term_dirs[n],dinfo[fi])))
            q1_raw = q_raw[Ysz,...]
            q = q1_raw[...,Xsz]
            ...
            q_plot = q_ax.pcolormesh(X,Y,q, cmap='jet', shading='gouraud',vmin=(np.nanmin(q)), vmax=(np.nanmax(q)))
            q_bar = q_fig.colorbar(q_plot)
            q_bar.set_title('Amplitude',fontsize=12)
            q_ax.set_title(label_full,fontsize=14)
            q_ax.set_xlabel(r'$\frac{X}{\lambda}$',fontsize=12)
            q_ax.set_ylabel(r'$\frac{Y}{\lambda}$',fontsize=12)
            filename = 'plots/'+term_names[n]+'/'+(''.join(filter(str.isdigit,str(dinfo[fi]))))+'.jpg'
            q_fig.savefig(os.path.join(out_path,filename))
...
def integrated_plot(Xgrid,Ygrid,term_dirs,term_names,out_path):
    ...
    for n in range(0,nterms):
        ...
        filename = term_names[n]+'_integrated_fft.jpg'
...
#transforms coordinates from lab frame to shock frame
    #X and Y grid size (Xgrid,Ygrid) must be grid size in lab frame
    #X grid in shock frame (Xsize) must be less than x distance between shock and edges of grid
    #Full pile path to directory containing extracted temperature files (t_dir) must be given
    #Variable directories (var_dirs) must be list of full paths to directories containing extracted variables to be coordinate transformed
    #Variable names (var_names) must be list of strings containing identifiable characters/names for each variable (eg. 'T' or 'Temperature')
    #Output path (out_path) must be full file path to directory in which output files will be created in a subdirectory called 'Coodtrans'
def coodtrans(Xgrid,Ygrid,Xsize,t_dir,var_dirs,var_names,out_path):
    #example: coodtrans(4096,4096,2000,'/localdata/scratch/adhav/Data/2_H2O2Ar_L16/Temperature/',['/localdata/scratch/adhav/Data/2_H2O2Ar_L16/Temperature/','/localdata/scratch/adhav/Data/2_H2O2Ar_L16/Pressure/'],['Temperature','Pressure'],'/localdata/scratch/adhav/Data/2_H2O2Ar_L16/')
    ...
    dinfo =  [f for f in os.listdir(t_dir) if os.path.isfile(os.path.join(t_dir,f))]
    nfiles = len(dinfo)
    shock = np.zeros([Ygrid,nfiles])
    nvars = len(var_dirs)

    for fi in range(0,nfiles):

        t = np.asarray(pd.read_csv(os.path.join(t_dir,dinfo[fi]),header=None))
    ...
    for n in range(0,nvars):
        ...
        if not os.path.isdir(os.path.join(out_path,'Coodtrans',var_names[n])):
            os.mkdir(os.path.join(out_path,'Coodtrans',var_names[n]))

        for fi in range(0,nfiles):

            q = np.asarray(pd.read_csv(os.path.join(var_dirs[n],dinfo[fi]),header=None))
            qtrans = np.zeros([Ygrid,Xsize])

            for i in range(0,(Ygrid-1)):
                for j in range((shock[i,fi]-Xsize),(shock[i,fi]+Xsize)):
                    a = j-shock[i,fi]-Xsize
                    qtrans[i,a]=q[i,j]

            filename = 'Coodtrans/'+var_names[n]+'/'+var_names[n][0]+(''.join(filter(str.isdigit,str(dinfo[fi]))))+'.csv'

            np.savetxt(os.path.join(out_path,filename),qtrans,delimiter=',')

#read_data(64800,4000,['/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/Temperature/','/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/Pressure/'],['Temperature','Pressure'],'/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/')

#raw_plots(13.65,0.84,64800,4000,['/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/Pressure/'],['Pressure'],['P [bar]'],'/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/')

#quantity(['0.5*T*P','T/(0.5*P)'],['TxP','TbyP'],['T','P'],['/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/Temperature/','/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/Pressure/'],'/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/')

#find_shock(64800,4000,'/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/Temperature/','/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Averages/')

#raw_plots(13.65,0.84,64800,4000,['/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/TbyP/','/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Reshaped/TxP/'],['TbyP','TxP'],['T/(0.5*P)','0.5*T*P'],'/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/')

proc_plots(64800,4000,13.65,0.84,1.45,['Temperature','Pressure'],[r'$\frac{T}{T_{ref}}$',r'$\frac{P}{P_{ref}}$'],['/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Averages/Tavg.csv','/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Averages/Pavg.csv'],['/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Fluctuations/Temperature/','/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/Fluctuations/Pressure'],'/localdata/scratch/adhav/Data/3_H2O2Ar_L13.65/')
